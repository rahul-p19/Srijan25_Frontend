    ...
    <div ref={container} className='w-full relative border-greyBorder border-t grid grid-cols-1 lg:grid-cols-5 grid-rows-3 lg:grid-rows-1 h-screen'>
      <div className='relative lg:bg-background z-[100] h-full row-span-1 border-greyBorder border-b lg:border-b-transparent'>
        ...
        <div className='absolute top-[65%] left-[50%] -translate-x-[50%] flex gap-x-3'>
          <button onClick={previousItem} className={`text-7xl font-bold ${timelineStart >= 0 ? 'opacity-40' : ''}`}>
            &lsaquo;
          </button>
          <button onClick={nextItem} className={`text-7xl font-bold ${window.innerWidth >= 1024 ? timelineStart <= (timelineEvents.length - 6) * -10 ? 'opacity-40' : '' : timelineStart <= (timelineEvents.length - 1) * -40 ? 'opacity-40' : ''}`}>
            &rsaquo;
          </button>
        </div>
        <img src='heading-outline.svg' alt="Outline for header" width="auto" height="auto" loading="lazy" className='px-2 scrollAnimatedText absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%]' />
      </div>
      ...
      <div className='absolute h-3/5 w-full bottom-0 lg:bottom-[20%] overflow-x-hidden row-span-2' onTouchStart={handleTouchStart} onTouchMove={handleTouchMove}>
        <div className='h-[2px] bg-white w-full absolute top-[50%]'></div>
        {timelineEvents.map((event, ind) => {
          const { left, top, lineTop } = getPosition(ind);

          return (
            <div key={ind}>
              <div className={`text-left text-nowrap absolute transition-all duration-1000`} style={{ left: `${left}%`, top: `${top}%` }}>
                {event}
              </div>
              <div className={`absolute w-[2px] h-[25%] bg-white transition-all duration-1000`} style={{ left: `${left + 2}%`, top: `${lineTop}%` }}></div>
              <div className={`absolute w-6 h-6 aspect-square rounded-full bg-white top-[50%] -translate-x-[50%] -translate-y-[50%] transition-all duration-1000`}
                style={{ left: `${left + 2}%` }}></div>
            </div>
          )
        })}
