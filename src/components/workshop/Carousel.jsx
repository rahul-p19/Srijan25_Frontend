...
function Carousel({ workshop }) {
  ...
  const previousItem = () => {
    if (carouselStart >= 0) window.innerWidth >= 600 ? setCarouselStart(-29 * (workshop.images.length - 1)) : setCarouselStart(-50 * (workshop.images.length));
  ...
  const nextItem = () => {
    if (window.innerWidth >= 600 && carouselStart <= (workshop.images.length - 2) * -25) setCarouselStart(0);
    else if (window.innerWidth < 600 && carouselStart <= (workshop.images.length - 1) * -50) setCarouselStart(0);
  ...
  return (
    <div ref={container} className='w-full relative border-greyBorder border-t grid grid-cols-1 sm:grid-cols-5 grid-rows-3 sm:grid-rows-1 h-[85vh] sm:h-[80vh] overflow-x-hidden'>
      <div className='relative bg-background z-[100] h-full border-greyBorder border-b sm:border-b-transparent sm:border-r sm:-mr-[1px] row-span-1'>
        ...
        <h2 className='scrollAnimatedText absolute text-2xl sm:text-xl xl:text-3xl text-center top-[50%] left-[50%] -translate-[50%]'>{workshop.name}</h2>
