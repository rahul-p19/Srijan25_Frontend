...
function Sponsors() {
  ...
  return (
    <div className='w-full relative border-white border-t grid grid-cols-5 h-[80vh] overflow-x-hidden'>
      ...
      <div className='relative col-span-3 border-white border-l border-r grid grid-cols-2'>
        ...
        <div className='absolute top-[50%] -translate-y-[50%] flex'>
          <ul className='flex gap-12 infinite-scroll-carousel bg-background border-white border-t border-b'>
            {heading === "Sponsors" ? [...currentSponsors, ...currentSponsors].map((sponsor, ind) => (
              <li key={ind} className='w-[30vw] grid place-items-center transition-all duration-1000s border-white border-l'>
